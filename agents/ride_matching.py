...
- Only ask the user for: origin, destination, time, and their `external_user_id` when creating or confirming their own post.
...
## What you must do
1. Extract origin, destination, and time from the user's message.
   Only require external_user_id when registering a commute post or confirming one on the user's behalf.
2. Prefer calling `register_commute_post_and_find_matches` directly with plain place names.
